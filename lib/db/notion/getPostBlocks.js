...
export async function fetchNotionPageBlocks(id, from = null) {
  ...
  if (await hydrateHtmlArtifacts(pageBlock)) {
...
export function hasExpiredSignedUrls(recordMap, bufferMs = 10 * 60 * 1000) {
  ...
  // New Notion HTML blocks are embeds backed by an attachment: URL. These
  // blocks need a signed URL just like files and PDFs; without one the browser
  // attempts to navigate to the unsupported attachment: scheme.
  const hasMissingSignedUrl = getNotionFileInstances(recordMap).some(
    ({ permissionRecord }) => !recordMap?.signed_urls?.[permissionRecord.id]
  )
  if (hasMissingSignedUrl) return true

  if (Object.values(recordMap?.signed_urls || {}).some(isExpiring)) return true

  // Newer Notion API responses can keep the signed URL on the block itself
  // instead of signed_urls. Refresh it before the cached URL expires.
  return Object.values(recordMap?.block || {}).some(entry => {
    const block = getBlockValue(entry)
    if (!block || !FILE_BLOCK_TYPES.includes(block.type)) return false
...
async function refreshSignedUrls(recordMap, cacheKey) {
  ...
  try {
    ...
    files.forEach((file, index) => {
      ...
      // Keep direct new-CDN file URLs current as well as signed_urls.
      const block = getBlockValue(recordMap?.block?.[id])
      const current = block?.type === 'page'
        ? block?.format?.page_cover
...
function getNotionFileInstances(recordMap) {
  return Object.values(recordMap?.block || {}).flatMap(entry => {
    const block = getBlockValue(entry)
    const isFileBlock = ['pdf', 'audio', 'image', 'video', 'file', 'page'].includes(block?.type)
    if (!block || (!isFileBlock && !isNotionHtmlArtifactBlock(block))) {
      return []
    }
...
function getBlockSource(block) {
  return block?.properties?.source?.[0]?.[0] || block?.format?.display_source
}

export function isNotionHtmlArtifactBlock(block) {
...
/**
 * Reads Notion's uploaded HTML attachment on the server and stores it on its
 * block. The client can then render the document through iframe srcDoc instead
 * of trying to launch the non-browser attachment: protocol.
 */
export async function hydrateHtmlArtifacts(recordMap) {
  const htmlBlocks = Object.values(recordMap?.block || {})
    .map(getBlockValue)
    .filter(isNotionHtmlArtifactBlock)

  if (!htmlBlocks.length) return false
  ...
  for (const block of htmlBlocks) {
    ...
    if (typeof block.format.html_artifact_content === 'string') continue
    ...
    try {
      ...
      if (!response.ok) {
        console.warn(
          `[Notion HTML block] failed to download ${block.id}: ${response.status}`
        )
        continue
      }

      const contentLength = Number(response.headers.get('content-length'))
      if (Number.isFinite(contentLength) && contentLength > HTML_ARTIFACT_MAX_BYTES) {
        console.warn(`[Notion HTML block] skipped oversized artifact: ${block.id}`)
        continue
      }

      const bytes = new Uint8Array(await response.arrayBuffer())
      if (bytes.byteLength > HTML_ARTIFACT_MAX_BYTES) {
        console.warn(`[Notion HTML block] skipped oversized artifact: ${block.id}`)
        continue
      }
      ...
      block.format.html_artifact_content = html
      changed = true
    } catch (error) {
      console.warn(`[Notion HTML block] failed to download ${block.id}:`, error)
    }
  }
