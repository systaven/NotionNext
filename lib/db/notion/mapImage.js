...
const mapImgUrl = (img, block, type = 'block', needCompress = true) => {
  ...
  // Notion's new file CDN links are signed and expire. Restore their stable
  // attachment identifier so the existing Notion image proxy can re-sign them.
  ...
  // 最后的域名替换（针对 Unsplash 代理）
  const unsplashHost = siteConfig('UNSPLASH_HOST')
  if (unsplashHost && ret && ret.includes('images.unsplash.com')) {
    ret = ret.replace('https://images.unsplash.com', unsplashHost)
  }
...
const compressImage = (image, width, quality = 50, fmt = 'webp') => {
  ...
  // Notion图床
  const unsplashHost = siteConfig('UNSPLASH_HOST')
  const isUnsplash = image.indexOf('https://images.unsplash.com/') === 0 || (unsplashHost && image.indexOf(unsplashHost) === 0)
  ...
  if (
    ...
    // 如果配置了代理，确保返回代理后的域名
    if (unsplashHost && result.includes('https://images.unsplash.com')) {
      result = result.replace('https://images.unsplash.com', unsplashHost)
    }
    return result
