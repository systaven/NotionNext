...
import { preparePostShortLinks } from '@/lib/shortLink/server'
...
export async function resolvePostProps({ prefix, slug, suffix, locale, from }) {
  ...
  if (post) {
    ...
    if (post?.type === 'Post') {
      try {
        post.shortLinkRoutes = await preparePostShortLinks(post.blockMap, post.id)
      } catch (e) {
        // Short links are an enhancement. Keep the existing Base64 route as a
        // safe fallback if Appwrite is temporarily unavailable.
        console.warn(`[${taskId}] [resolvePostProps] preparePostShortLinks failed`, e)
      }
    }
...
async function convertNotionToSiteData(
  ...
  const inferredCollectionId =
    collectionMap && typeof collectionMap === 'object' && Object.keys(collectionMap).length === 1 ? Object.keys(collectionMap)[0] : null
  const collectionId = rawMetadata?.collection_id || inferredCollectionId
  const rawCollection = collectionId
    ? collectionMap?.[collectionId] || (typeof collectionId === 'string' ? collectionMap?.[idToUuid(collectionId)] : null) || {}
    : {}
...
function getSiteInfo({ collection, block, rawMetadata, NOTION_CONFIG }) {
  ...
  const description = collection?.description?.[0]?.[0] || defaultDescription
  // 站点封面优先级：
